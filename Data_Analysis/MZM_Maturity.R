...
final <- ADV001 %>% 
  select(Location_Name, DESIGNATION, Days_to_maturity)%>% 
  filter(Location_Name== "ANGONIA")%>%
  group_by(DESIGNATION)%>%
  summarize(Days_to_maturity=mean(Days_to_maturity))
...
data_new <- final %>% 
  ...
  arrange(desc(Days_to_maturity)) %>% 
  slice(1:5)


data_new1 <- final1 %>% 
  select(Variety, Days_to_maturity,)%>% 
  arrange(Days_to_maturity) %>% 
  slice(1:5)
...
x1=ggplot(data_new, aes(x= Variety, y= Days_to_maturity, fill = Variety)) + 
  geom_bar(position="dodge", stat="identity") + coord_flip()

x2=ggplot(data_new1, aes(x= Variety, y= Days_to_maturity, fill = Variety)) + 
  geom_bar(position="dodge", stat="identity") + coord_flip()

x3=ggplot(data_new2, aes(x= Variety, y= Days_to_maturity, fill = Variety)) + 
  geom_bar(position="dodge", stat="identity") + coord_flip()

x4=ggplot(data_new3, aes(x= Variety, y= Days_to_maturity, fill = Variety)) + 
  geom_bar(position="dodge", stat="identity") + coord_flip()

x5=ggplot(data_new4, aes(x= Variety, y= Days_to_maturity, fill = Variety)) + 
  geom_bar(position="dodge", stat="identity") + coord_flip()

x6=ggplot(data_new5, aes(x= Variety, y= Days_to_maturity, fill = Variety)) + 
  geom_bar(position="dodge", stat="identity") + coord_flip()

x7=ggplot(data_new6, aes(x= Variety, y= Days_to_maturity, fill = Variety)) + 
  geom_bar(position="dodge", stat="identity") + coord_flip()

x8=ggplot(data_new7, aes(x= Variety, y= Days_to_maturity, fill = Variety)) + 
  geom_bar(position="dodge", stat="identity") + coord_flip()

x9=ggplot(data_new8, aes(x= Variety, y= Days_to_maturity, fill = Variety)) + 
  geom_bar(position="dodge", stat="identity") + coord_flip()
